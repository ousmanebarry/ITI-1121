...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    private int size;
    private Node<E> tail;
    private Node<E> lastNodeGLobal;

    public int count(E fromElement, E toElement) {

        if (lastNodeGLobal == null) {
            lastNodeGLobal = head;
        }

        Node<E> newFromElement = firstOccurence(fromElement, lastNodeGLobal);

        if (fromElement == null || newFromElement == null) {
            return 0;
        }
        ...
        if (fromElement == toElement) {
            lastNodeGLobal = null;
            return 1;
        } else if (fromElement == tail.value && fromElement != toElement) {
            lastNodeGLobal = null;
            return 1;
        } else {
            lastNodeGLobal = newFromElement.next;
            return count(lastNodeGLobal.value, toElement) + 1;
        }

    }

    private Node<E> firstOccurence(E element, Node<E> lastNode) {

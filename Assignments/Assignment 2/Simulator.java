...
public class Simulator {
	...
	public Simulator(ParkingLot lot, int perHourArrivalRate, int steps) {
		...
		// https://stackoverflow.com/questions/8406250/variable-of-interface-type
	...
	public void simulate() {
		...
		while (clock < steps) {
			...
			if (RandomGenerator.eventOccurred(probabilityOfArrivalPerSec)) {
				Spot incomingCar = new Spot(RandomGenerator.generateRandomCar(), clock);
				incomingQueue.enqueue(incomingCar);
			}
			...
			for (int i = 0; i < lot.getNumRows(); i++) {
				for (int j = 0; j < lot.getNumSpotsPerRow(); j++) {
					if (lot.getSpotAt(i, j) != null) {
						...
						int duration = lot.getSpotAt(i, j).getTimestamp();

						if (duration == MAX_PARKING_DURATION) {
							outgoingQueue.enqueue(lot.remove(i, j));
						} else if (duration < MAX_PARKING_DURATION) {
							Rational probabilityDeparture = departurePDF.pdf(duration);

							if (RandomGenerator.eventOccurred(probabilityDeparture)) {
								outgoingQueue.enqueue(lot.remove(i, j));
							}
						}
					}
			...
			if (!incomingQueue.isEmpty()) {
				...
				if (dequeuedCar == null) {
					ourCar = incomingQueue.dequeue();
				} else {
					ourCar = dequeuedCar;
				}

				if (lot.attemptParking(ourCar.getCar(), ourCar.getTimestamp())) {
					System.out.println(
							ourCar.getCar() + " ENTERED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
				} else {
					dequeuedCar = ourCar;
				}
			...
			if (!outgoingQueue.isEmpty()) {
				...
				System.out
						.println(c.getCar() + " EXITED at timestep " + clock + "; occupancy is at " + lot.getTotalOccupancy());
			}
